RSpec.describe NxtHttpClient::Client do
  ...
  context 'is switched off per default' do
    ...
    it 'can be switched on per request' do
      ...
      expect(subject.post(http_stats_url('200'), cache: :thread)).to_not be_cached
      expect(subject.post(http_stats_url('200'), cache: :thread)).to be_cached
      expect(subject.post(http_stats_url('200'), cache: :thread)).to be_cached

      # not yet cached and thus does not hit the cache the first time
      Thread.new { expect(subject.fire(http_stats_url('200'), cache: :thread)).to_not be_cached }.join
      Thread.new { expect(subject.fire(http_stats_url('200'), cache: :thread)).to_not be_cached }.join
    end
  end
end
